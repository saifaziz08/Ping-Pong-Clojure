...
(defn draw []
      ...
      (draw-rect @ball)
      (get-pixel)
      (q/text (str  "Score Left (" @left-side-score ") Vs Right (" @right-side-score ")") 60 20 120 60)
      )
...
;(def right-side-score (atom 0))
(defn reset-score []
      (reset! left-side-score  0)
      (reset!  right-side-score  0)
      )
...
(defn execute []
      (q/defsketch pong
                   :title "2d pong game"
                   :size [450 200]
                   :setup (fn [] (smooth) (no-stroke) (frame-rate @frame-r) (reset-score))
